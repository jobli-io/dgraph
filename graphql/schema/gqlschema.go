...
enum DateTimeGranularity { year month day week hour }
...
func completeSchema(
	...
	for _, key := range definitions {
		...
		addInterfaceMemberTypesEnum(sch, defn) // must run before addFilterType
...
// addInterfaceMemberTypesEnum generates an `enum XxxTypename { ImplA ImplB ... }` type
// for each interface that has at least one implementing concrete type. This enum is used
// by the `memberTypes` filter field on the interface's XxxFilter input to allow callers
// to restrict an interface query to a subset of implementing types, e.g.:
//
//	queryManageable(filter: { memberTypes: [Workspace, Group] }) { ... }
func addInterfaceMemberTypesEnum(schema *ast.Schema, defn *ast.Definition) {
	if defn.Kind != ast.Interface {
		return
	}
	implementors := schema.PossibleTypes[defn.Name]
	if len(implementors) == 0 {
		return
	}
	enumName := defn.Name + "Typename"
	enum := &ast.Definition{
		Kind: ast.Enum,
		Name: enumName,
	}
	names := make([]string, 0, len(implementors))
	for _, impl := range implementors {
		names = append(names, impl.Name)
	}
	sort.Strings(names)
	for _, name := range names {
		enum.EnumValues = append(enum.EnumValues, &ast.EnumValueDefinition{Name: name})
	}
	schema.Types[enumName] = enum
}
...
func addFilterType(schema *ast.Schema, defn *ast.Definition, providesTypeMap map[string]bool) {
	...
	// For interfaces: prepend a memberTypes field so callers can scope the query to a
	// subset of implementing types, e.g. filter: { memberTypes: [Workspace, Group] }.
	// The backing enum (XxxTypename) was generated by addInterfaceMemberTypesEnum.
	// Skip injection if the interface itself already declares a field called memberTypes
	// to avoid generating a duplicate field in the filter input type.
	if defn.Kind == ast.Interface {
		if enumName := defn.Name + "Typename"; schema.Types[enumName] != nil &&
			defn.Fields.ForName("memberTypes") == nil {
			filter.Fields = append(filter.Fields, &ast.FieldDefinition{
				Name: "memberTypes",
				Type: &ast.Type{Elem: &ast.Type{NamedType: enumName}},
			})
		}
	}
