...
type xidMetadata struct {
	...
	// blankNodeResolutions maps a blank-node UID (e.g. "_:User_3") to a real Dgraph UID
	// (e.g. "0xABC") or a real blank-node (e.g. "_:User_5"). This is used to patch
	// dangling forward-ref entries after an existing node was discovered via a different
	// @id field (Case 3: existing user found by xId but forward-ref was emitted by sId).
...
// resolveExactFwdRef records that the forward-ref blank node blankNode
// (e.g. "_:User_3") should be replaced by realUID (e.g. "0xABC") during
// patchBlankNodes. It only records if the blank node is not already resolved.
func (xm *xidMetadata) resolveExactFwdRef(blankNode, realUID string) {
	if _, already := xm.blankNodeResolutions[blankNode]; !already {
		xm.blankNodeResolutions[blankNode] = realUID
	}
}
...
func patchBlankNodes(v interface{}, resolutions map[string]string) {
	switch node := v.(type) {
	case map[string]interface{}:
		if uidVal, ok := node["uid"]; ok {
			if blankNode, isStr := uidVal.(string); isStr {
				if realUID, resolved := resolutions[blankNode]; resolved {
					node["uid"] = realUID
				}
			}
		}
		for _, child := range node {
			patchBlankNodes(child, resolutions)
		}
	case []interface{}:
		for _, item := range node {
			patchBlankNodes(item, resolutions)
		}
	}
}
...
func (arw *AddRewriter) Rewrite(
	...
	for _, frag := range arw.frags {
		...
		// emitted for a different @id field of the same type, e.g. xId vs sId).
		if len(xidMetadata.blankNodeResolutions) > 0 {
			patchBlankNodes(frag.fragment, xidMetadata.blankNodeResolutions)
		}
			...
			if xidVal, ok := obj[xid.Name()]; ok && xidVal != nil {
				...
				if interfaceUidExist || typUidExist {
					...
					if atTopLevel {
						...
						// further.
						if typUidExist {
							// Case 3 fix: existing node found via one @id field (e.g. xId → 0xABC).
							// A different @id field (e.g. sId, generated by @default on the parent
							// type's @defaults loop) may have already emitted a forward-ref blank node
							// BEFORE this field was resolved (alphabetical field ordering means
							// "createdBy" is visited before "ownedBy").
							//
							// To resolve the right forward refs (and not accidentally resolve refs
							// for other nodes of the same type), we re-evaluate the @default
							// expressions for every @id field of this type using the current obj.
							// Each computed XID value uniquely identifies WHICH forward ref (if any)
							// should be patched to point to the found node's real UID.
							for _, xid := range typ.XIDFields() {
								if _, present := obj[xid.Name()]; present {
									// XID value already in obj — skip; it was already used in Phase-1
									// and would have been handled via idExistence directly.
									continue
								}
								...
								defStr, xerr := extractVal(defVal, xid.Name(), xid.Type().Name())
								if xerr != nil || defStr == "" {
									continue
								}
								// varGen.Next is idempotent for values already registered.
								xidVar := varGen.Next(typ, xid.Name(), defStr, false)
								if blankNode, hasFwd := xidMetadata.forwardRefs[xidVar]; hasFwd {
									xidMetadata.resolveExactFwdRef(blankNode, typUid)
								}
							}
